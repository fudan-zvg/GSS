...
class ToUniversalLabel(object):
    def __init__(self, dataset_name: str, use_naive_taxonomy: bool = False) -> None:
    ...
    def __call__(self, results):
        for key in results.get('seg_fields', []):
            gt_seg = results[key]
            # print('cjq debug ToU gt_seg', key, gt_seg.type())
            gt_seg = torch.tensor(gt_seg).to(torch.long)
            gt_seg = self.tax_converter.transform_label(gt_seg, self.dataset_name).numpy()
            results[key] = gt_seg
        # results['dataset_name'] = self.dataset_name
        # print('cjq debug transform results', results)
        return results
