...
class BigSegAggHeadRelaxE08HungerEdit(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 palette=None,
                 category_indice=None,
                 indice_seg_channel=None,
                 **kwargs):
        super(BigSegAggHeadRelaxE08HungerEdit, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
        num_inputs = len(self.in_channels)
        assert num_inputs == len(self.in_index)
        self.channels = channels
        self.palette = torch.tensor(palette)
        self.category_indice = torch.tensor(category_indice).cuda()
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        with torch.no_grad():
            ...
            # edit the indice of central area
            gt_local_min = - F.max_pool2d(- gt_semantic_seg_for_recon.float(), kernel_size=(32, 32), stride=(4, 4))
            gt_local_min = F.interpolate(gt_local_min.float(), size=gt_semantic_seg_indices.shape[-2:], mode='bilinear')
            gt_local_max = F.max_pool2d(gt_semantic_seg_for_recon.float(), kernel_size=(32, 32), stride=(4, 4))
            gt_local_max = F.interpolate(gt_local_max.float(), size=gt_semantic_seg_indices.shape[-2:], mode='bilinear')
            edit_mask = torch.zeros_like(gt_local_min).cuda()
            edit_mask[gt_local_max == gt_local_min] = 1
            edited_gt_semantic_seg_indices = gt_semantic_seg_indices.clone()
            gt_local_max[gt_local_max == self.pixel_ignore_index] = self.num_classes
            edited_gt_semantic_seg_indices[edit_mask == 1] = self.category_indice[gt_local_max[edit_mask == 1].long()]
            ...
            final_gt_semantic_seg_indices = edited_gt_semantic_seg_indices.clone()
            final_gt_semantic_seg_indices[indice_map_mask == 0] = self.indice_ignore_index
            ...
            save_image(torch.cat([
                ...
                torch.Tensor.repeat(F.interpolate(indice_map_mask.float(), size=gt_semantic_seg.shape[-2:]), 3, 1, 1, 1, 1
                                    ).squeeze(2).permute(1, 0, 2, 3),
                torch.Tensor.repeat(F.interpolate(gt_semantic_seg_indices.float(), size=gt_semantic_seg.shape[-2:]), 3, 1, 1, 1,
                                    1).squeeze(2).permute(1, 0, 2, 3) / 8192.0,
                torch.Tensor.repeat(F.interpolate(final_gt_semantic_seg_indices.float(), size=gt_semantic_seg.shape[-2:]), 3, 1, 1, 1,
                                    1).squeeze(2).permute(1, 0, 2, 3) / 8192.0,
                torch.Tensor.repeat(
                    F.interpolate(vq_logits.argmax(1).unsqueeze(1).float(), size=gt_semantic_seg.shape[-2:]), 3, 1, 1, 1,
                    1).squeeze(2).permute(1, 0, 2, 3) / 8192.0,
            ],
                dim=0), 'work_dirs/edit_bigseg_ade20k_conns_swin_160k_show/show_train_16k/train_' + img_metas[0]['ori_filename'].split('/')[-1],
                        nrow=len(pixel_pred_from_indice_pred) * 10)
            print('cjq save images')
        losses = self.losses(indice_seg_logit=vq_logits,
                             pixel_seg_logit=pixel_logits,
                             masked_indice_seg_label=final_gt_semantic_seg_indices,
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        ...
        #                      dim=0), 'work_dirs/edit_bigseg_ade20k_conns_swin_160k/show_160k/val_160k_' + img_metas[0]['ori_filename'].split('/')[-1])
        # print('cjq save images')
    ...
    def _forward_test_recon_with_dalle(self, gt_semantic_seg, img_metas):
        ...
        for gt_semantic_seg_item in gt_semantic_seg:
            ...
            # edit the indice of central area
            # print('cjq debug', gt_semantic_seg_item.shape)
            gt_local_min = - F.max_pool2d(- gt_semantic_seg_item.float(), kernel_size=(32, 32), stride=(4, 4))
            gt_local_min = F.interpolate(gt_local_min.unsqueeze(0).float(), size=input_ids.shape[-2:], mode='bilinear')
            gt_local_max = F.max_pool2d(gt_semantic_seg_item.float(), kernel_size=(32, 32), stride=(4, 4))
            gt_local_max = F.interpolate(gt_local_max.unsqueeze(0).float(), size=input_ids.shape[-2:], mode='bilinear')
            edit_mask = torch.zeros_like(gt_local_min).cuda()
            edit_mask[gt_local_max == gt_local_min] = 1
            edited_input_ids = input_ids.clone().unsqueeze(0).cuda()
            gt_local_max[gt_local_max == self.pixel_ignore_index] = self.num_classes
            edited_input_ids[edit_mask == 1] = self.category_indice[gt_local_max[edit_mask == 1].long()]

            rec_segmap = self.d_vae.decode(edited_input_ids.squeeze(0), img_size=[h, w])
            rec_segmap = unmap_pixels(torch.sigmoid(rec_segmap[:, :3])) * 255
            ...
            #                       map_pixels(self.encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0),
            #                       torch.Tensor.repeat(
            #                           F.interpolate(edit_mask, size=gt_semantic_seg[0].shape[-2:], mode='bilinear'),
            #                           3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)],
            #                      dim=0), 'work_dirs/edit_bigseg_ade20k_conns_swin_160k/edit_gt_show/val_' + img_metas[0]['ori_filename'].split('/')[-1])
            # print('cjq save images')
