...
class DecoderEmbeddings(nn.Module):
    def __init__(self, vocab_size, hidden_dim, padding_idx, max_position_embeddings, dropout=0):
        super().__init__()
        self.word_embeddings = nn.Embedding(
            vocab_size, hidden_dim,padding_idx)
...
class BigSegAggHeadWoCPMTransformerAutoreg(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerAutoreg, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
        ...
        self.vit_num_head = vit_num_head
        ...
        decoder_layer = nn.TransformerDecoderLayer(d_model=self.vit_hidden_dim, nhead=self.vit_num_head)
        self.transformer_block = nn.TransformerDecoder(
            decoder_layer = decoder_layer, 
            num_layers = self.vit_num_layers, 
            norm=None
        )
        self.embeddings = DecoderEmbeddings(self.vocab_size+2,self.vit_hidden_dim,self.vocab_size,64*64+1,)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        seq = gt_semantic_seg_indices.view(b,-1) 
        seq = torch.cat([torch.ones((b, 1)).to(seq) * 8193, seq[:, :-1]], dim=1)
        tgt = self.embeddings(seq)
        tgt = tgt.transpose(0,1)
        tgt_mask = (torch.tril(torch.ones([h*w,h*w]),diagonal=0)>0).cuda() #mask
        x = x.flatten(2).permute(2,0,1)

        tgt_pred = self.transformer_block(tgt,x,tgt_mask)
        tgt_pred = tgt_pred.permute(1,2,0).view(b, -1, h, w)
        tgt_pred = self.conv_before_seg(tgt_pred)
        vq_logits = self.forward(tgt_pred).view(-1, self.vocab_size, h, w)
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        ...
        b, c, h, w = x.shape
        x = x.flatten(2).permute(2,0,1)
        seq = torch.ones(b,1).to(x).int()*8193
        #import pdb;pdb.set_trace()
        for i in range(h*w):
            tgt = self.embeddings(seq).transpose(0,1)   #[N,B,C]
            tgt_mask = (torch.ones([i+1,i+1])!=0).cuda() #mask
            tgt_pred = self.transformer_block(tgt,x,tgt_mask)
            tgt_pred = tgt_pred.permute(1,2,0).unsqueeze(-1)  #[B,C,N]
            tgt_pred = self.conv_before_seg(tgt_pred)
            vq_logist = self.forward(tgt_pred)
            vq_indice = torch.argmax(vq_logist,dim = 1,)[:,-1]
            seq = torch.cat((seq,vq_indice),1)
        #x = self.swin_ln(x)
