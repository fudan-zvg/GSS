...
class BigSegAggHeadWoCPMTransformerDepthUpHSV(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 swin_window_size=7,
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerDepthUpHSV, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
        ...
        self.conv_up = upConvLayer(channels, channels)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        if not x_p.shape[-2:] == (h * 8, w * 8):
            x_p = self.conv_up(x_p, (h * 8, w * 8))
        h_p, w_p = x_p.shape[-2:]
        pixel_depth = self.conv_seg_pixel(x_p).view(-1, 1, h_p, w_p)
        pixel_depth = pixel_depth.sigmoid() * self.max_depth
        ...
        with torch.no_grad():
            ...
            depth_h, s, v = gt_semantic_seg_for_recon, torch.ones_like(gt_semantic_seg_for_recon), torch.ones_like(
                gt_semantic_seg_for_recon)
            depth_h = 1 - depth_h
            hsv_depth = torch.cat([depth_h, s, v], dim=1)
            rgb_depth = self.convertor.hsv_to_rgb(hsv_depth)
            gt_semantic_seg_for_recon = rgb_depth
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        ...
        # depth_pred = rec_segmap.mean(1).unsqueeze(0)
        rec_hsv_depth = self.convertor.rgb_to_hsv(rec_segmap)
        depth_pred = 1 - rec_hsv_depth[:,0:1,:,:]
        ...
        self.count += 1
        self.mean_rmse += rmse
        mrmse = self.mean_rmse.item() / self.count
        print('\n RMSE:', mrmse)
        ...
        # save_image(rec_segmap, 'work_dirs/nyu_depth_pred/iter_12k_tf_corr_rgb/val_com' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_recmap.png')
        save_image(self.depth_norm(gt_semantic_seg[0].unsqueeze(0)), 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_gt.png')
        save_image(self.depth_norm(depth_pred), 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_pred.png')
        # save_image(torch.cat([self.depth_norm(gt_semantic_seg[0].unsqueeze(0)),
        #                       self.depth_norm(depth_pred)],
        #                      dim=0), 'work_dirs/nyu_depth_pred/iter_12k_tf_corr_rgb/val_com' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_pred_gt.png')
        # print('cjq save images')
        # save_image(rec_segmap, 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + 'img.png')

        # save_image(rec_segmap, 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_colormap_{:.2f}.png'.format(rmse))
        # save_image(torch.cat([self.depth_norm(gt_semantic_seg[0].unsqueeze(0)),
        #                       self.depth_norm(depth_pred)],
        #                      dim=0), 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_pred_gt_{:.2f}.png'.format(rmse))
        print('cjq save images')
        return depth_pred
    ...
    def _forward_test_recon_with_dalle(self, gt_semantic_seg, img_metas):
        ...
        depth_h, s, v = gt_semantic_seg_item, torch.ones_like(gt_semantic_seg_item), torch.ones_like(gt_semantic_seg_item)
        depth_h = 1 - depth_h
        hsv_depth = torch.cat([depth_h, s, v], dim=0).unsqueeze(0)
        ...
        save_image(rec_segmap, 'work_dirs/nyu_depth_pred/iter_56k_tf_corr_rgb_corr_hsv/' +
                   img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_colormap_gt.png')
