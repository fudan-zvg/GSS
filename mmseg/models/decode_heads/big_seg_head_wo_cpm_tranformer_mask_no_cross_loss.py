...
class BigSegAggHeadWoCPMTransformerMaskNoCrossLoss(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 swin_window_size=7,
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerMaskNoCrossLoss, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
        ...
        self.conv_seg_pixel = nn.Conv2d(self.indice_seg_channel, self.num_classes, kernel_size=1)
    ...
    def forward_train(self, inputs_list, img_metas, gt_semantic_seg, train_cfg):
        inputs,ori_img,mask = inputs_list
        inputs = self._transform_inputs(inputs)
        x = self.feature_aggregation(inputs)
        recon_img = self.recon(x)
    ...
    def forward_test(self, inputs_list, img_metas, gt_semantic_seg, test_cfg):
        ...
        # save_image(self.encode_to_segmap_visual(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0,
        #            'work_dirs/visualization_stage_2/cityscapes/' +
        #            img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '_pred.png')
        # save_image(self.encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0,
        #            'work_dirs/visualization_stage_2/cityscapes/' +
    ...
    def _forward_test_recon_with_dalle(self, gt_semantic_seg, img_metas):
        ...
        for gt_semantic_seg_item in gt_semantic_seg:
            ...
            # error[gt_semantic_seg[0] >= self.num_classes] = 0
            # error[error > 0] = 1
            # save_image(map_pixels(self.encode_to_segmap(gt_semantic_seg[0].long()) / 255.0),
            #            'work_dirs/dalle_rgb_recon_ade20k/val_' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[
            #                0] + '_rgb.png')
            # save_image(map_pixels(self.encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0),
            #            'work_dirs/dalle_rgb_recon_ade20k/val_' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[
            #                0] + '_rgb_recon.png')
            # gt_semantic_seg[0] += 1
            # gt_semantic_seg[0][gt_semantic_seg[0] > self.num_classes] = 0
            # save_image(gt_semantic_seg[0] / gt_semantic_seg[0].max(),
            #            'work_dirs/dalle_rgb_recon_ade20k/val_' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[
            #                0] + '_category_index_norm.png')
            # # save_image(torch.cat([map_pixels(self.encode_to_segmap(gt_semantic_seg[0].long()) / 255.0),
            # #                       map_pixels(self.encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0),
            # #                       torch.Tensor.repeat(error, 3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)],
    ...
    def losses(self,
        ...
        recon_img = resize(
            ...
            mode='bilinear',
            align_corners=self.align_corners)
        recon_img = recon_img.view(recon_img.shape[0],recon_img.shape[1],-1).transpose(1,2)
        ori_img = ori_img.view(ori_img.shape[0],ori_img.shape[1],-1).transpose(1,2)
        extended_mask = torch.repeat_interleave(recon_mask,int(ori_img.shape[1]/recon_mask.shape[1]),dim = 1).unsqueeze(-1).expand_as(ori_img)
        #import pdb;pdb.set_trace()
        # Waiting for test.
        b,c,h_indice,w_indice = masked_indice_seg_label.shape
        masked_indice_seg_label = masked_indice_seg_label.view(b,c,-1).transpose(1,2)
        extender_mask_for_indice = torch.repeat_interleave(recon_mask,int(masked_indice_seg_label.shape[1]/recon_mask.shape[1]),dim = 1).unsqueeze(-1).expand_as(masked_indice_seg_label)
        masked_masked_indice_seg_label = masked_indice_seg_label*extender_mask_for_indice+self.indice_ignore_index*(1-extender_mask_for_indice)
        masked_masked_indice_seg_label = masked_masked_indice_seg_label.view(b,h_indice,w_indice).long()
        masked_indice_seg_label = masked_indice_seg_label.view(b,h_indice,w_indice)

        b,c,h_pixel,w_pixel = pixel_seg_label.shape
        pixel_seg_label = pixel_seg_label.view(b,c,-1).transpose(1,2)
        extender_mask_for_pixel = torch.repeat_interleave(recon_mask,int(pixel_seg_label.shape[1]/recon_mask.shape[1]),dim = 1).unsqueeze(-1).expand_as(pixel_seg_label)
        masked_pixel_seg_label = pixel_seg_label*extender_mask_for_pixel+self.pixel_ignore_index*(1-extender_mask_for_pixel)
        masked_pixel_seg_label = masked_pixel_seg_label.view(b,h_pixel,w_pixel).long()
        pixel_seg_label = pixel_seg_label.view(b,h_pixel,w_pixel)
            ...
            if loss_decode.loss_name not in loss:
                if loss_decode.loss_name == 'loss_ce':
                    ...
                    loss[loss_decode.loss_name] = loss_decode(
                        ...
                        ignore_index=self.indice_ignore_index)
                elif loss_decode.loss_name == 'loss_recon':
                    loss[loss_decode.loss_name] = loss_decode(
                        recon_img*(1-extended_mask),
                        ori_img*(1-extended_mask))
            else:
                if loss_decode.loss_name == 'loss_ce':
                    loss[loss_decode.loss_name] += loss_decode(
                        indice_seg_logit,
