...
class BigSegAggHeadWoCPMTransformerNonAutoreg(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerNonAutoreg, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
        ...
        self.embeddings = DecoderEmbeddings(1,self.vit_hidden_dim,None,64*64+1,)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        seq = torch.zeros_like(gt_semantic_seg_indices.view(b,-1))
        ...
        # import ipdb
        # ipdb.set_trace()
        tgt_pred = self.transformer_block(tgt,x)
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        ...
        b, c, h, w = x.shape
        seq = torch.zeros(b, h*w).long().cuda()
        # import ipdb
        # ipdb.set_trace()
        tgt = self.embeddings(seq)
        tgt = tgt.transpose(0,1)
        ...
        tgt_pred = self.conv_before_seg(tgt_pred)
        vq_logits = self.forward(tgt_pred).view(-1, self.vocab_size, h, w)
        vq_indices = vq_logits.argmax(1).unsqueeze(1)
