...
class BigSegAggHeadWoCPMTransformerSingleFusion(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 swin_window_size=7,
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerSingleFusion, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        b, c, h, w = x.shape
        pixel_logits = self.conv_seg_pixel(x).view(-1, self.num_classes, h, w)
        # swin block
        x = x.flatten(2).transpose(1, 2)
        x, hw, x_down, hw_down = self.transformer_block(x, (h, w))
        x = self.swin_ln(x)
        x = x.transpose(1, 2).view(b, c, h, w)
        # small dim to 2048 vector
