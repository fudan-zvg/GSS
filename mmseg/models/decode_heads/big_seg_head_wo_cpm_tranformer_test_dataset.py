...
class BigSegAggHeadWoCPMTransformerTestDataset(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 swin_window_size=7,
                 **kwargs):
        super(BigSegAggHeadWoCPMTransformerTestDataset, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def forward_train(self, inputs_list, img_metas, gt_semantic_seg, train_cfg):
        inputs,img,mask = inputs_list
        import pdb;pdb.set_trace()
        save_image(gt_semantic_seg.squeeze(1).permute(0,3,1,2).float()/255,'work_dirs/bigseg_rain_swin_40k_test_dataset/show_img_gt/'+img_metas[0]['ori_filename'].split('/')[-1])
        save_image(img/255,'work_dirs/bigseg_rain_swin_40k_test_dataset/show_img/'+img_metas[0]['ori_filename'].split('/')[-1])
