...
class EditIndice(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(EditIndice, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        gt_semantic_seg = torch.ones_like(gt_semantic_seg) * self.index
        ...
        with torch.no_grad():
            ...
            print('cjq save images', max(gt_semantic_seg_indices), min(gt_semantic_seg_indices))
            self.list.append(gt_semantic_seg_indices)
            self.index += 1
        losses = self.losses(indice_seg_logit=vq_logits,
                             pixel_seg_logit=pixel_logits,
                             masked_indice_seg_label=gt_semantic_seg_indices,
                             full_indice_seg_label=gt_semantic_seg_indices,
                             pixel_seg_label=gt_semantic_seg,
                             pred_pixel_from_pred_indice=pixel_logits)
        return losses
