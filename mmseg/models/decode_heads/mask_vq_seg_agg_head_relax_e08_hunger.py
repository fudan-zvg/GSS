...
class MaskVQSegAggHeadRelaxE08Hunger(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MaskVQSegAggHeadRelaxE08Hunger, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _forward_test_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        seg_logist = F.one_hot(seg_pred.to(torch.int64), self.num_classes + 1).squeeze(1).permute(0, 3, 1, 2).to(
            torch.float)
        ...
        #                       map_pixels(encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0),
        #                       torch.Tensor.repeat(error, 3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)],
        #                      dim=0), 'work_dirs/mask_vqseg_agg_swin_large_patch4_window12_768x768_pretrain_384x384_22K_300e_cityscapes_relax_e08_hunger/show_val_20/val_' + img_metas[0]['ori_filename'].split('/')[-1])
    ...
    def _forward_train_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        print('cjq debug vqseg:', vq_logits.shape)
