...
class MaskVQSegAggHeadRelaxE08HungerDualLoss(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MaskVQSegAggHeadRelaxE08HungerDualLoss, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _forward_train_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        with torch.no_grad():
            ...
            # 在原本的v1中 仅仅选择了最窄的区域进行correct pixel区域进行ignore，本实验对这个区域进行松弛，纳入更大的范围进行监督
            # 对pixel预测正确的局部进行松弛, 将正确像素标记为0，错误像素为1，便于进行腐蚀， 在v3和本版本中使用该方案
            ...
            # ignore_map[pred_pixel == gt_pixel] = 0
            # unlabel area will be ignored
            ...
            gt_semantic_seg_indices_sparce = gt_semantic_seg_indices.clone()
            gt_semantic_seg_indices_dense = gt_semantic_seg_indices.clone()
            gt_semantic_seg_indices_sparce[indice_map_mask == 0] = self.ignore_index
            gt_semantic_seg_indices_dense[ignore_mask == 0] = self.ignore_index
            # error map
        ...
        losses = self.losses(vq_logits, gt_semantic_seg_indices_sparce.clone().detach(), gt_semantic_seg_indices_sparce.clone().detach())
    ...
    def losses(self, seg_logit, seg_label, seg_label_2=None):
        ...
        if seg_label_2 is not None:
            seg_label_2 = seg_label_2.squeeze(1)
