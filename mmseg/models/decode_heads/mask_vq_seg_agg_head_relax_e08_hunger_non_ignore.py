...
class MaskVQSegAggHeadRelaxE08HungerNonIgnore(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MaskVQSegAggHeadRelaxE08HungerNonIgnore, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _feature_aggregation(self, inputs):
        ...
        return out, out_for_pixel

    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        # Receive 4 stage backbone feature map: 1/4, 1/8, 1/16, 1/32
        x, x_pixel = self._feature_aggregation(inputs)
        if self.d_vae_type == 'dalle':
            result =  self._forward_train_seg_with_dalle(x, x_pixel, gt_semantic_seg, img_metas)
            return result
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        with torch.no_grad():
            ...
            if self.d_vae_type == 'dalle' and self.task_type == 'recon':
                ...
                return self._forward_test_seg_with_dalle(x, x_pixel, gt_semantic_seg, img_metas)
    ...
    def _forward_test_seg_with_dalle(self, inputs, inputs_pixel, gt_semantic_seg, img_metas):
        h, w = inputs.shape[-2:]
        ...
        rec_segmap = unmap_pixels(torch.sigmoid(rec_segmap[:, :3])) * 255
        seg_pred = decode_from_segmap(rec_segmap, num_classes=19)
        ...
        # save images
        h_p, w_p = inputs_pixel.shape[-2:]
        # get indices logist from ViT
        vq_logits = self.forward(inputs).view(-1, self.vocab_size, h, w)
        # get vq indices from gt by dalle
        pixel_logits = self.conv_seg_pixel(inputs_pixel).view(-1, self.num_classes, h_p, w_p)
        pixel_logits = F.interpolate(pixel_logits, size=(h_p * 4, w_p * 4), mode='bilinear')
        pixel_pred = pixel_logits.argmax(1).unsqueeze(1)

        final_gt_semantic_seg = torch.tensor(gt_semantic_seg[0].clone()).long()
        final_gt_semantic_seg[torch.tensor(gt_semantic_seg[0] == 255)] = pixel_pred[torch.tensor((gt_semantic_seg[0] == 255)).unsqueeze(0)]
        ...
        save_image(torch.cat([encode_to_segmap(gt_semantic_seg[0].long()) / 255.0,
                              rec_segmap / 255.0,
                              encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0,
                              encode_to_segmap(pixel_pred.long()) / 255.0,
                              encode_to_segmap(final_gt_semantic_seg.long()) / 255.0,
                              torch.Tensor.repeat(error, 3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)],
                             dim=0), 'work_dirs/mask_vqseg_agg_swin_large_patch4_window12_768x768_pretrain_384x384_22K_300e_cityscapes_relax_e08_hunger_non_ignore/show_with_pixel_pred_and_final_pred/val_' + img_metas[0]['ori_filename'].split('/')[-1])
    ...
    def _forward_train_seg_with_dalle(self, inputs, inputs_pixel, gt_semantic_seg, img_metas):
        h, w = inputs.shape[-2:]
        h_p, w_p = inputs_pixel.shape[-2:]
        # get indices logist from ViT
        vq_logits = self.forward(inputs).view(-1, self.vocab_size, h, w)
        ...
        pixel_logits = F.interpolate(pixel_logits, size=(h_p * 4, w_p * 4), mode='bilinear')
        with torch.no_grad():
            # 求无ignore区域的gt_semantic_seg_for_recon，用于重建出无ignore区域的gt_indice：gt_semantic_seg_indices
            pixel_pred = pixel_logits.argmax(1).unsqueeze(1)
            gt_semantic_seg_for_recon = torch.zeros_like(gt_semantic_seg)
            gt_semantic_seg_for_recon[gt_semantic_seg != 255] = gt_semantic_seg[gt_semantic_seg != 255].clone()
            gt_semantic_seg_for_recon[gt_semantic_seg == 255] = pixel_pred[gt_semantic_seg == 255].clone()
            gt_semantic_seg_indices = self.get_gt_vq_indices(gt_semantic_seg_for_recon).unsqueeze(1) # % 100

            # 求e08松弛的正确像素掩码：relaxation_map
            ...
            relaxation_map[pred_pixel == gt_semantic_seg_for_recon] = 0
            ...
            # 求gt中的ignore区域掩码，并在gt_indice上，遮蔽正确像素区域和ignore区域，得到gt_semantic_seg_indices
            ...
            gt_semantic_seg_indices[indice_map_mask == 0] = self.ignore_index
        ...
        losses = self.losses(vq_logits, gt_semantic_seg_indices.clone().detach(), pixel_logits, gt_semantic_seg.clone().detach())
    ...
    def losses(self, seg_logit, seg_label, pixel_logit, pixel_seg_label):
