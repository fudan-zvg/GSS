...
class MaskVQSegAggHeadV1_1(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MaskVQSegAggHeadV1_1, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _forward_train_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        with torch.no_grad():
            ...
            # 在v1中会把已经预测正确的pixel给取消监督，在这个版本中，我们不取消这些监督，作为对照。
