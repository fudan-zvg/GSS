...
class MaskVQSegAggHeadV2(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MaskVQSegAggHeadV2, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _forward_test_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        #                       map_pixels(encode_to_segmap(seg_logist.argmax(1).unsqueeze(1).long()) / 255.0),
        #                       torch.Tensor.repeat(error, 3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)],
        #                      dim=0), 'work_dirs/mask_vqseg_agg_v2_swin_large_patch4_window12_768x768_pretrain_384x384_22K_300e_cityscapes/show_19_cls_deocde/val_' + img_metas[0]['ori_filename'].split('/')[-1])
        ...
        #
        # sio.savemat('work_dirs/mask_vqseg_agg_v2_swin_large_patch4_window12_768x768_pretrain_384x384_22K_300e_cityscapes/anal_19/' + img_metas[0]['ori_filename'].split('/')[-1].split('.')[0] + '.mat',
    ...
    def _forward_train_seg_with_dalle(self, inputs, gt_semantic_seg, img_metas):
        ...
        with torch.no_grad():
            # 确保gt_semantic_seg 的尺寸是indice尺寸的8倍，先映射到logist空间，再进行双线性插值，最后压缩到一层
            gt_semantic_seg[gt_semantic_seg == 255] = self.num_classes
            ...
            # 获取indices，这个就是所有监督信号，但是有一部分是噪声，需要剔除
            gt_semantic_seg_indices = self.get_gt_vq_indices(gt_semantic_seg).unsqueeze(1) # % 100
            ignore_indice_mask = self.get_ignore_mask(gt_semantic_seg, gt_semantic_seg_indices)
            gt_semantic_seg_indices[ignore_indice_mask == 0] = self.ignore_index
            # error map
    ...
    def get_ignore_mask(self, gt_semantic_seg, gt_vq_indices):
        ...
        # # 获取像素级的预测值，用于判断是否已经达到合格要求 (这部分代码经过分析，弃用)
