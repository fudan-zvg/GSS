...
class SwinTransformer(BaseModule):
    ...
    def forward(self, x, guide_code=None):
        if self.patch_embed:
            x, hw_shape = self.patch_embed(x)
        else:
            x, hw_shape = x, (x.shape[-2], x.shape[-1])
            x = x.flatten(2).transpose(1, 2)
            if self.norm is not None:
                x = self.norm(x)
        if guide_code is not None:
            guide_code = resize(guide_code, hw_shape, mode='bilinear')
            guide_code = guide_code.flatten(2).transpose(1, 2)
            x = x + guide_code
        if self.use_abs_pos_embed:
