...
class VQSegSlowAggHead(BaseDecodeHead):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(VQSegSlowAggHead, self).__init__(init_cfg=init_cfg, input_transform='multiple_select', channels=channels, **kwargs)
    ...
    def _forward_test_recon_with_dalle(self, gt_semantic_seg, img_metas):
        for gt_semantic_seg_item in gt_semantic_seg:
            ...
            gt_semantic_seg_item = F.one_hot(gt_semantic_seg_item.to(torch.int64), self.num_classes + 1)\
                .permute(0, 3, 1, 2).to(torch.float)
            results = []
            for i in range(self.num_classes):
                cls_gt = gt_semantic_seg_item[:, i:i+1, : ,: ] * 255
                cls_gt_rgb = torch.Tensor.repeat(cls_gt, 3, 1, 1, 1, 1).squeeze(2).permute(1, 0, 2, 3)
                input_segmap = map_pixels(cls_gt_rgb / 255.0)
                ...
                rec_segmap = torch.sigmoid(rec_segmap[:, :3]) * 255
                rec_segmap[rec_segmap < 128] = 0
                rec_segmap[rec_segmap >= 128] = 1
                rec_cls_gt = rec_segmap[:,0,:,:]
                results.append(rec_cls_gt)
            seg_logist = torch.stack(results, dim=1)
